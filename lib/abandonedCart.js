  ...
  const cartLines = cartItems
    .map((item) => {
      ...
      if (item.isCustomPack) {
        const entries = Array.isArray(item.customPackResolved?.entries)
          ? item.customPackResolved.entries
          : [];
        const split =
          entries.length > 0
            ? ` (${entries
                ...
                .join(", ")})`
            : "";
        return `- Custom 12-Pack${split} x${qty}`;
      }
...
const getCartTotal = (cartItems, discountsEnabled) =>
  cartItems.reduce((sum, item) => {
    if (item.isCustomPack) {
      const resolved = item.customPackResolved;
      if (!resolved || resolved.error) {
        return sum;
      }
      return sum + Number(resolved.totalPrice || 0);
    }
...
const runAbandonedCartReminders = async () => {
  ...
  for (const user of candidates) {
    ...
    const customProductIds = Array.from(
      new Set(
        cartItemsRaw.flatMap((item) =>
          item.isCustomPack
            ? parseCustomPackConfig(item.customPackConfig).map((entry) => entry.productId)
            : [],
        ),
      ),
    );
    ...
    const cartItems = cartItemsRaw.map((item) => {
      ...
      return {
        ...item,
        customPackResolved: resolveCustomPack({
